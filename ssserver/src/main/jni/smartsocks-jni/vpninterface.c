...
void vpn_interface_protect_socket(int socket)
{
    ...
    clazz =(*g_env)->FindClass(g_env, "com/kkt/ssserver/SSProxyServer");
    if (NULL == clazz) {
        return;
    }

    protectSocketFdMethod = (*g_env)->GetStaticMethodID(
            g_env, clazz, "protectSocket", "(I)V");
    if (NULL == protectSocketFdMethod) {
        return;
    }

    (*g_env)->CallStaticVoidMethod(
            g_env,clazz, protectSocketFdMethod, (jint)socket);
}
